...
- `POST /api/entries/file`：`session`（本机省略）、`path`、`name`，创建空文件；名称必须有主体及扩展名，拒绝同名目标。
- `POST /api/entries/folder`：`session`、`path`，自动创建“新建文件夹”“新建文件夹1”…，返回实际名称。
- `POST /api/entries/folder/rename`：`session`、`path`、`name`、`newName`，同目录文件夹改名，拒绝目标冲突。

业务实现在 `EntryCreationService`。普通 FTP 不提供标准的原子“不覆盖改名”指令，远端通过会话串行操作和发布前检查避免本项目并发冲突；不要让其他客户端同时修改同一目标名称。
