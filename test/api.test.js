...
test('local directory metadata and invalid directory',async()=>{const dir=await fs.mkdtemp(path.join(os.tmpdir(),'ftpview-test-'));try{await fs.mkdir(path.join(dir,'文件夹'));await fs.writeFile(path.join(dir,'a.txt'),'hello');const r=await invoke('/api/local/list',{query:{path:dir}});assert.equal(r.status,200);assert.equal(r.data.files[0].type,'folder');assert.equal(r.data.files[1].size,5);assert.equal((await invoke('/api/local/list',{query:{path:path.join(dir,'missing')}})).status,400);}finally{await fs.rm(dir,{recursive:true});}});
